...
def copy_template(
    ...
    try: from copier import run_copy
    except ImportError as exception:
        raise _exceptions.DependencyAbsence( 'copier' ) from exception
    copy_kwargs: dict[ str, __.typx.Any ] = dict(
        data = _acquire_answers_file( answers_file ),
        defaults = True,
        overwrite = True,
        quiet = True,
    )
...
def execute_validation_commands(
    ...
    for cmd in config.commands:
        ...
        _execute_command( args, cwd, temp_dir, config.preserve )
